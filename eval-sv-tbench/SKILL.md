...
1. Confirm inputs.
If `eval_name` is not provided, call `mcp__eaas-siliconmcp__list_evals` and ask user to choose one. Ask for optional `session_name`, `output_dir`, and `notify_email`. Use `session_name = output_dir` or `eval_name` if not specified.

3. List testcases.
Call `mcp__eaas-siliconmcp__list_eval_testcases(eval_name)` and capture the testcase names.

4. Load testcase data and stage files.
...
8. Request notification and fetch results.
